...
public class JumpLink : MonoBehaviour {
	...
	void Update () {
        if (gameObject.CompareTag("JUMPLINK")) {
            if (loopNumber == 0 && Input.GetKey(KeyCode.Alpha2)) {
                KeyCode = KeyCode.Alpha2;
                NextLoopNumber = 1;
                gameObject.layer = 8; // ACTIVE
            }

            if (loopNumber == 1 && Input.GetKey(KeyCode.Alpha1)) {
                KeyCode = KeyCode.Alpha1;
                NextLoopNumber = 0;
                gameObject.layer = 8; // ACTIVE
            }

            if (loopNumber == 1 && Input.GetKey(KeyCode.Alpha3)) {
                KeyCode = KeyCode.Alpha3;
                NextLoopNumber = 2;
                gameObject.layer = 8; // ACTIVE
            }

            if (loopNumber == 2 && Input.GetKey(KeyCode.Alpha2)) {
                KeyCode = KeyCode.Alpha2;
                NextLoopNumber = 1;
                gameObject.layer = 8; // ACTIVE
            }
